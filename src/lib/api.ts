...
export const api = {
  ...
  /** Les requêtes déjà exécutées, la plus ancienne en premier. Entrées
   * complètes (date et connexion), contrairement aux historiques de shell qui
   * ne rendent que les commandes : ici on affiche « quand » et « où ». */
  sqlHistory: () => invoke<CommandEntry[]>("get_sql_history"),
  appendSqlHistory: (query: string, connection: string | null) =>
    invoke<void>("append_sql_history", { query, connection }),
  ...
  /** Applies one edit to several hosts. Only the fields present in `edit` are
   * written — see `BulkEdit`. `secret` is stored only when `auth` is being
   * changed, since a password means nothing without its method. */
  bulkEditHosts: (hostIds: HostId[], edit: BulkEdit, secret: string | null) =>
    invoke<Workspace>("bulk_edit_hosts", { hostIds, edit, secret }),
  ...
  /** Les champs d'un item avec leurs valeurs, quand le panneau le déplie. */
  guivaultBrowseItem: (vaultId: VaultId, id: string) => invoke<GuiVaultBrowseValue[]>("guivault_browse_item", { vaultId, id }),
  /** La valeur d'un champ, au moment de la copier ou la coller (la palette). */
  ...
  /** Une session SSO de `~/.aws/config` et ses profils → un accès AWS du
   * coffre (mis à jour s'il y est déjà). */
  guivaultAwsSaveSession: (name: string) => invoke<GuiVaultAwsSaved>("guivault_aws_save_session", { name }),
  /** Un accès AWS du coffre (lu par `guivaultBrowse`) → `~/.aws/config`. */
  guivaultAwsApply: (vaultId: VaultId, id: string) => invoke<GuiVaultAwsApplied>("guivault_aws_apply", { vaultId, id }),
  ...
  listPersistentSessions: (hostId: HostId) => invoke<SessionListing>("list_persistent_sessions", { hostId }),
  /** Termine une session et rend la liste à jour — même forme que
  ...
  copyEntries: (sourcePaneId: string, sourceCwd: string, entries: Entry[], destPaneId: string, destCwd: string, conflict: ConflictPolicy) =>
    invoke<string>("copy_entries", { sourcePaneId, sourceCwd, entries, destPaneId, destCwd, conflict }),
  /** Les noms déjà pris dans le dossier de destination — à demander avant de
   * copier, pour poser la question plutôt qu'écraser en silence. */
  checkCopyConflicts: (destPaneId: string, destCwd: string, names: string[]) =>
    invoke<CopyConflict[]>("check_copy_conflicts", { destPaneId, destCwd, names }),
  ...
  /** Taille récursive d'un dossier, calculée du côté où il vit (`du` sur un
   * hôte, un parcours Rust pour le panneau local). Jamais appelée pour tout
   * un listing d'office — un `du` par dossier, c'est ce que ça coûte. */
  paneDirSize: (paneId: string, path: string) => invoke<number>("pane_dir_size", { paneId, path }),
  /** Recherche récursive par nom sous `root`, bornée en profondeur, en
   * résultats et en durée (voir `termius_core::pane_ops`). Rend des chemins :
   * le panneau navigue jusqu'à celui qu'on choisit. */
  paneFind: (paneId: string, root: string, pattern: string) =>
    invoke<PaneFindOutcome>("pane_find", { paneId, root, pattern }),
  /** Archive une sélection dans le dossier courant, sur place — rien ne
   * transite par le réseau, même pour un hôte distant. Refuse d'écraser une
   * archive existante. */
  ...
  /** Compare les arborescences des deux panneaux sous leurs dossiers
   * courants : un inventaire par côté (une commande chacun), puis la
   * comparaison. Bornée en profondeur et en nombre de fichiers ; le dit
   ...
   * comme pour une invite d'authentification expirée. */
  answerRunbookApproval: (runId: string, stepIndex: number, approved: boolean) =>
    invoke<void>("answer_runbook_approval", { runId, stepIndex, approved }),
...
/** Une étape s'est arrêtée pour demander l'accord : **rien n'est parti**.
 * Sans réponse au bout de `timeoutSecs`, c'est un refus. */
export function onRunbookApprovalNeeded(
  handler: (request: RunbookApprovalRequest) => void,
): Promise<UnlistenFn> {
  return listen<RunbookApprovalRequest>("runbook-approval-needed", (event) => handler(event.payload));
}
